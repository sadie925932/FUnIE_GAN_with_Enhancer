...
class GetValImage(Dataset):
    ...
    def get_file_paths(self, root, dataset_name):
        if dataset_name=='EUVP':
            ...
            files = sorted(glob.glob(os.path.join(root, 'validation_A') + "/*.*"))

        return files

class GetNewValImage(Dataset):
    """ Common data pipeline to organize and generate
         training pairs for various datasets
    """
    def __init__(self, root, dataset_name, transforms_=None):
    ...
    def __getitem__(self, index):
        ...
        img_A = self.transform(img_A)
        img_B = self.transform(img_B)
        return {"val_A": img_A, "val_B": img_B}
    ...
    def get_file_paths(self, root, dataset_name):
        ...
        if dataset_name =='EUVP':
            sub_dirs = ['underwater_imagenet', 'underwater_dark', 'underwater_scenes']
            for sd in sub_dirs:
                filesA += sorted(glob.glob(os.path.join(root, sd, 'validation_A') + "/*.*"))
                filesB += sorted(glob.glob(os.path.join(root, sd, 'validation_B') + "/*.*"))
        else:
            filesA += sorted(glob.glob(os.path.join(root, 'validation_A') + "/*.*"))
            filesB += sorted(glob.glob(os.path.join(root, 'validation_B') + "/*.*"))
        return filesA, filesB
